...
| FR-043 | IMPLEMENTED | `ProjectsRepository.createTask`, `TaskFormDialog` | API-080 | DB-070 | `tasks.create` | TEST-007 | PHASE-015 | GATE-015 | Ações de reuniões vinculáveis e criáveis como tarefas operacionais |
...
| FR-070 | IMPLEMENTED | `apps/api/src/modules/projects/*`, `apps/web/src/modules/projects/*` | API-040 (`GET/POST /projetos`) | DB-070 (`projects`) | `projects.create/read` | TEST-005 | PHASE-015 | GATE-015 | Gestão de projetos com cliente, responsáveis, prazo e orçamento |
| FR-071 | IMPLEMENTED | `ProjectDetailPage.tsx`, `GanttChart.tsx` | API-041 (`GET /projetos/:id/tarefas`) | DB-070 (`tasks`) | `tasks.read` | TEST-005 | PHASE-015 | GATE-015 | Alternância de visões Lista, Kanban e gráfico de Gantt |
| FR-072 | IMPLEMENTED | `projects.repository.ts`, `TaskFormDialog.tsx` | API-041 (`POST /tarefas`) | DB-070 (`tasks`) | `tasks.create/update` | TEST-005 | PHASE-015 | GATE-015 | Subtarefas com `parent_task_id`, prioridades e prazos |
