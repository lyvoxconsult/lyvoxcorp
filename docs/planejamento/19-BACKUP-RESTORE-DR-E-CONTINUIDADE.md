...
- **Fontes Consultadas:** PROMPT-FINAL-UNICO-OTTERCRAFT, pgBackRest Manual, PostgreSQL Disaster Recovery Guide
...
> 
> Arquivos de backup armazenados exclusivamente na mesma VPS onde a aplicação roda **não oferecem proteção real** contra falha física do host ou destruição de disco. Todos os backups do **Lyvox Gerenciamento** são criptografados e transferidos para um **Armazenamento Offsite Isolado** (Bucket S3 compatível configurável).

---

## 2. Metas de Continuidade de Negócio (RPO e RTO)

- **Recovery Point Objective (RPO):** **MÁXIMO DE 1 HORA**. Em caso de desastre total, a perda máxima aceitável é de 1 hora de dados (garantida via replicação contínua de logs WAL do PostgreSQL com pgBackRest).
...
graph TD
    subgraph Host_Producao[VPS Produção]
        DB[(PostgreSQL 16 DB)] -->|pgBackRest WAL / Dump| LocalBackup[/var/backups/lyvox/]
        Storage[/var/lib/lyvox/storage/] -->|Restic Criptografado| LocalFiles[/var/backups/files/]
    end

    subgraph Offsite_Storage[Armazenamento Secundário Offsite]
        LocalBackup -->|Upload Criptografado AES-256| OffsiteBucket[(Bucket Offsite S3 Compatível)]
...
|---|---|---|---|---|---|
| **PostgreSQL Full** | Diário (02:00h) | `pgBackRest` | AES-256 | 7 dias | 30 dias |
| **PostgreSQL WAL / Incremental**| A cada 1 hora | `pgBackRest` WAL Archiving | AES-256 | 24 horas | 7 dias |
| **Arquivos Privados (Storage)**| Diário (03:00h) | `restic` | AES-256 | 3 dias | 30 dias |
| **Configurações e Env Schema** | Semanal | Dump Criptografado | AES-256 | 30 dias | 90 dias |

---

## 4. Runbook Detalhado de Restauração de Desastre (Restore Runbook)

### 4.1 Cenário: Perda Total da VPS Principal
...
2. **Passo 2 — Importação de Chaves:** Importar as chaves GPG/AES salvas no cofre de segredos corporativo.
3. **Passo 3 — Download do Backup Offsite:** Baixar o último backup Full do PostgreSQL e arquivos do bucket offsite:
   ```bash
...
6. **Passo 6 — Subida dos Contêineres:** Executar `docker compose up -d` e validar os endpoints de healthcheck (`/health` e `/readiness`).
