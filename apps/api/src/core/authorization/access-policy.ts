...
export type AccessPolicy =
  ...
  | { kind: 'PERMISSIONS'; permissions: readonly string[]; allowedScopes: readonly ResourceScope[] };
...
export function RequirePermissions(...permissions: string[]) {
  if (permissions.length < 2 || permissions.some((permission) => !isPermissionKey(permission))) {
    throw new Error('Invalid compound permission metadata');
  }
  return SetMetadata(ACCESS_POLICY, { kind: 'PERMISSIONS', permissions, allowedScopes: ['ALL'] } satisfies AccessPolicy);
}
