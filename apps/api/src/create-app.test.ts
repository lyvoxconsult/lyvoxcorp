...
describe('createApp', () => {
  ...
  it('serves generated OpenAPI 3.1 and Swagger UI only at the root docs path', async () => {
    ...
    expect(document.paths).toHaveProperty('/api/v1/crm/leads');
    expect(document.paths).toHaveProperty('/api/v1/crm/leads/import');
    expect(document.paths).toHaveProperty('/api/v1/crm/leads/{id}/stage');
    expect(document.paths).toHaveProperty('/api/v1/crm/leads/{id}/followups');
    expect(document.paths).toHaveProperty('/api/v1/crm/leads/{id}/convert');
    expect(document.paths).toHaveProperty('/api/v1/crm/stages');
    expect(document.paths).toHaveProperty('/api/v1/crm/responsaveis');
    ...
    expect(document.paths['/api/v1/crm/leads'].post.requestBody.content['application/json'].schema.properties).toHaveProperty('estimatedValue');
    expect(document.paths['/api/v1/crm/leads'].get.parameters).toEqual(expect.arrayContaining([
      expect.objectContaining({ name: 'search', in: 'query' }), expect.objectContaining({ name: 'stageId', in: 'query' }),
      expect.objectContaining({ name: 'responsibleId', in: 'query' }), expect.objectContaining({ name: 'cursor', in: 'query' }),
      expect.objectContaining({ name: 'pageSize', in: 'query' }),
    ]));
    expect(document.paths['/api/v1/crm/leads'].post.parameters).toEqual(expect.arrayContaining([expect.objectContaining({ name: 'Idempotency-Key', required: true })]));
    expect(document.paths['/api/v1/crm/leads/{id}/stage'].patch.parameters).not.toEqual(expect.arrayContaining([expect.objectContaining({ name: 'Idempotency-Key' })]));
    expect(document.paths['/api/v1/crm/leads/{id}/stage'].patch.responses['200'].content['application/json'].schema.required).toEqual(expect.arrayContaining(['id', 'stageId', 'version']));
    expect(document.paths['/api/v1/crm/leads/{id}/convert'].post.responses['200'].content['application/json'].schema.required).toEqual(expect.arrayContaining(['leadId', 'clientId', 'version']));
    expect(document.paths['/api/v1/crm/leads/import'].post.responses['200'].content['application/json'].schema.properties).toHaveProperty('errors');
